  ...
  "node_modules/front-matter/index.js"(exports, module2) {
    ...
    function extractor(string, options) {
      ...
      if (lines[0] && /= yaml =|---/.test(lines[0])) {
        return parse2(string, options.allowUnsafe);
      } else {
        return {
          attributes: {},
          body: string,
...
var unknown = "<unknown>";
var ThrottledOctokit = import_utils.GitHub.plugin(import_plugin_throttling.throttling);
function isConditionMet(templateString, templateVars) {
  return new Function(`return ${templateString};`).call(templateVars);
}
async function getLabelsFromConfig(newFm, oldFm, config) {
...
async function run() {
  const GITHUB_TOKEN = import_core.default.getInput("token");
  const octokit = new ThrottledOctokit((0, import_utils.getOctokitOptions)(GITHUB_TOKEN, { throttle: {
    onRateLimit: (retryAfter, options) => {
      var _a;
      octokit.log.warn(`Request quota exhausted for request ${(options == null ? void 0 : options.method) || unknown} ${(options == null ? void 0 : options.url) || unknown}`);
      if (((_a = options == null ? void 0 : options.request) == null ? void 0 : _a.retryCount) <= 2) {
        console.log(`Retrying after ${retryAfter} seconds!`);
        return true;
      }
    },
    onSecondaryRateLimit: (_retryAfter, options) => octokit.log.warn(`Abuse detected for request ${(options == null ? void 0 : options.method) || unknown} ${(options == null ? void 0 : options.url) || unknown}`)
  } }));
  const payload = import_github.default.context.payload;
  const { repository, pull_request } = payload;
  const configFile = import_core.default.getInput("config-path") || ".jekyll-labels.yml";
  const response = await octokit.request(`GET /repos/${repository.owner.login}/${repository.name}/contents/${configFile}`);
  if (response.status !== 200) {
    import_core.default.setFailed(`Could not find config file at ${configFile}`);
    process.exit(1);
  }
  ...
  await Promise.all(fetched.map(async function(file) {
    ...
    if (file.status === "removed" || file.status === "modified") {
      const response2 = await octokit.request(`GET /repos/${repository.owner.login}/${repository.name}/contents/${file.filename}`);
      const content = Buffer.from(response2.data.content, "base64").toString("utf8");
      oldFm = (0, import_front_matter.default)(content).attributes;
    }
    if (file.status === "added" || file.status === "modified") {
      const response2 = await octokit.request(`GET /repos/${pull_request.base.repo.owner.login}/${pull_request.base.repo.name}/contents/${file.filename}?ref=${pull_request.head.sha}`);
      const content = Buffer.from(response2.data.content, "base64").toString("utf8");
      newFm = (0, import_front_matter.default)(content).attributes;
    }
  ...
  await Promise.all(issueLabels.map(async (label) => {
    import_core.default.info(`Found label: ${label}`);
    if (!labels.has(label)) {
      import_core.default.info(`Label ${label} should not be applied`);
      try {
        ...
        import_core.default.info(`Removed label ${label}`);
      } catch {
        import_core.default.warning(`Could not remove label ${label}`);
      }
    } else {
      import_core.default.info(`Added label ${label}`);
    }
  }));
}
run();
